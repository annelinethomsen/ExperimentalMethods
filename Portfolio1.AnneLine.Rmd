...
The 'filter function' is used to make a new  dataframe, where the participans with no dominant eye (the ones who answered "Both") are sorted out, to make the results more accurate. 

```{r question1}
Data_ocular_rl <- filter(Full_personality_data, Full_personality_data$ocular_dom == "Right" | Full_personality_data$ocular_dom == "Left")
```

This is how the ggplot looks of the new dataframe.
...
####Should I explain what we basically and visually see in the graph???

The summarise-function is used to find the mean and the standard deviation for each of the groups.
...
This is how a bar plot of the looks like. 
...
The summarise-function is used to find the mean and the standard deviation for each gender.
```{r}
...
??? Should I look for outliers, now that the standard deviation is so high?
---
...
To visually investigate the distribution of the breath_hold data a frequency distribution and a qqplot is made.
...
To clean the data from outliers every observation that is above the mean + 2 * sd is removed from the dataset.

```{r removing outliers}
Full_personality_data$breath_hold_zscores <- ((Full_personality_data$breath_hold - mean(Full_personality_data$breath_hold)) / sd(Full_personality_data$breath_hold))

Breath_hold_filtered <- filter(Full_personality_data, Full_personality_data$breath_hold_zscores < 2 & Full_personality_data$breath_hold_zscores > -2)
```

```{r plotting data without outliers}
ggplot(Breath_hold_filtered, aes(x=breath_hold)) + geom_histogram(aes(y = ..density..), colour = "black", fill = "white") + stat_function(fun = dnorm, args = list(mean = mean(Breath_hold_filtered$breath_hold), sd = sd(Breath_hold_filtered$breath_hold)), colour = "red", size = 1)
```
...
```{r making qq-plot of data outliers removed}
qplot(sample = Breath_hold_filtered$breath_hold) + labs(x="Expected values", y="Observed value")
```

#How do I add the straight line with the ideal normal distribution???
Right now I have kept both codes with and without outliers, because i'm not sure whether to include them or not. 

```{r include=FALSE}
install.packages("pastecs")
```

To investigate the breath hold data numeric the function stat.desc is used.
...
Since the skew has to be under 1, and the kurtosis as close to 0 as possible, and the p-value has to be under the trehshold of 0.05, I would say this distribution is normal even though the p-value is under 0.05, cause it's very close to 0.05. (??)

#Aren't these results a bit contradicting???

```{r calculating confidence intervals}
(mean(Breath_hold_filtered$breath_hold) + 1.96 * sd(Breath_hold_filtered$breath_hold)) - (mean(Breath_hold_filtered$breath_hold) - 1.96 * sd(Breath_hold_filtered$breath_hold))
```
...
ggplot(Full_personality_data, aes(x=shoesize, y=breath_hold)) + geom_point(colour = "Black") + geom_smooth()
```

??? What to conclude out of this graph?
??? What does the hint mean?

```{r grouping data by gender}
Group_gender <- Full_personality_data %>% group_by(gender)
#R read 'Group_gender' as a value, so the it's manually set as a data.frame.
Group_gender<-as.data.frame(Group_gender)
```

```{r making scatterplot}
ggplot(Group_gender, aes(x = shoesize, y = breath_hold, colour = gender)) + geom_point() + geom_smooth(method = "lm")
```

```{r making scatterplot with one linear model}
ggplot(Group_gender, aes(x = shoesize, y = breath_hold)) + geom_point(aes(colour=gender)) + geom_smooth(method = "lm")
```

Explain your results in plain terms here: 
When looking at this scatterplot it seems that for the females there is a correlation between showsize and the time one can hold her breath; the bigger the shoe size, the longer the breath_hold. 
Opposed to the females this data indicates that for males there's no particular correlation between 
The only reason the linear model representing both genders are not Is because males on average can hold their breath longer than girls.
This indicates that there might not be a correlation 
