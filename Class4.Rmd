...
```{r}
rplot <- read.csv("NEW_CogSciPersonalityTest2019 (1).csv")
```
...
People who prefer cola S hear more music per week than people who prefer cola L.
...
```{r}
ggplot(rplot, aes(x = taste_cola, y = hours_music_per_week, fill = taste_cola)) + geom_boxplot(width = 0.5) + ggtitle("Box Plot with mean") + stat_summary(fun.y = mean, geom = "point", shape = 23, colour = "Black")
```
...
```{r}
ggplot(rplot[rplot$hours_music_per_week<30,], aes(x = taste_cola, y = hours_music_per_week, fill = taste_cola)) + geom_boxplot(width = 0.5) + ggtitle("Box Plot with mean") + stat_summary(fun.y = mean, geom = "point", shape = 23, colour = "Black")
```

```{r}
rplot$z <- ((rplot$hours_music_per_week - mean(rplot$hours_music_per_week)) / sd(rplot$hours_music_per_week))

#We can now see in our new collumn, that Tobias is an outlier

rplot_filtered <- filter(rplot, rplot$z < 3 & rplot$z > -3)
```
...
```{r}
ggplot(rplot[rplot$hours_music_per_week<30,], aes(x = taste_cola, y = hours_music_per_week, fill = taste_cola)) + geom_boxplot(width = 0.5) + 
  geom_errorbar(aes(ymin = mean(rplot$hours_music_per_week) - sd(rplot$hours_music_per_week), ymax = mean(df$ratio2d4d) + sd(df$ratio2d4d)), width = 0.1, color = 'black') +
  ggtitle("Box Plot with mean") + stat_summary(fun.y = mean, geom = "point", shape = 23, colour = "Black")
```
