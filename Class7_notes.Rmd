...
Already now you should try to answer the following questions:
  ...
  All the variables above are continious, which allows us to make a regression. 
  
  What kind of statistical tests can you use to explore this kind of data? Could you run a t-test?
  - Standard simple regression (requires two continious variables)
  - We can also run correlation
  - We CAN NOT make a t-test, because it requires a categorical variable. 
...
```{r}
#plot the data
ggplot(child, aes(Television, Aggression)) + geom_point() + ggtitle('Scatter plot')
```
...
#plot the line
ggplot(child, aes(Television, Aggression)) + geom_point() + geom_abline(intercept = int, slope = slope) + ggtitle('Scatter plot with the regression line: manually')
```
...
ggplot(child, aes(Television, Aggression)) + geom_point() + geom_smooth(method = lm) + ggtitle('Scatter plot with the regression line: quicker using geom_smooth (method lm)')

#same but without the confidence intervals around the line, so it looks more like the abline geom
ggplot(child, aes(Television, Aggression)) + geom_point() + geom_smooth(method = lm, se = FALSE) + ggtitle('Scatter plot with the regression line: geom_smooth without confidence intervals ')
```
...
```{r}
ggplot(child, aes(Television, Aggression)) + geom_point() + geom_smooth(method = lm, se = FALSE) + ggtitle('Scatter plot with the regression line')

ggplot(child, aes(Computer_Games, Aggression)) + geom_point() + geom_smooth(method = lm, se = FALSE) + ggtitle('Scatter plot with the regression line')

ggplot(child, aes(Sibling_Aggression, Aggression)) + geom_point() + geom_smooth(method = lm, se = FALSE) + ggtitle('Scatter plot with the regression line')

ggplot(child, aes(Diet, Aggression)) + geom_point() + geom_smooth(method = lm, se = FALSE) + ggtitle('Scatter plot with the regression line')

ggplot(child, aes(Parenting_Style, Aggression)) + geom_point() + geom_smooth(method = lm, se = FALSE) + ggtitle('Scatter plot with the regression line')
```

Which one of predictors was the worst? 
- Diet is the worst (because it's just horisontal)
- Parenting_style might be the most steep of thegraphs
...
- I can't have pacman in this version of R...
```{r}
pacman::p_load(ggplot2, reshape2)

#plot as heatmap
qplot(x=Var1, y=Var2, data=melt(cor(child, use="p")), fill=value, geom="tile") + scale_fill_gradient2(limits=c(-1, 1))
