...
```{r Finding the class of the collumn balloon_balance}
class("balloon_balance")
```

The class is "character"

```{r Fingding the class of the collumn inside the dataframe}
...
```{r}
groupby_hand <- group_by(df, handedness)

summarise(n_hand=n(), groupby_hand, (mean(tongue_twist)))

#  <chr>         <int>                  <dbl>
#1 Ambidextrous      1                   50  
#2 Left-handed       8                   50.7
#3 Right-handed     53                   45.4

#The mean is more reliable when the number of people is higher. (The more samples, the more accurate model)
```
