...
#Putting logfiles together in one dataframe. 
data <- lapply(files, read.csv) %>% rbind.fill()


#We have two different collumns with reaction time, and we want to put these together, which we do by making a new collumn called "RT2". (If there's data in "Reaction_Time" then put this into the new collumn and if there's data in "Reaction.time" then put this into the new collumn)
data$RT2 <- ifelse(is.na(data$Reaction.time), data$Reaction_Time, data$Reaction.time) 

#Now we want to sort out the two collumns "Reaction_Time" and "Reaction.time":
...
t.test(RT2 ~ Gender, data = data3)
# t = 7.4726, p-value = 9.201e-14
```
...
t.test(RT2 ~ Gender, data = data3, var.equal = T)
# t = 7.254, p-value = 4.655e-13
```
...
t.test(RT2 ~ Gender, data = data3, paired = T)
#It doesn't work, because not all arguments have the same length
```
...
t.test(data3$RT2, mu = 0.5)
#t = -1.4024, p-value = 0.1608

t.test(data3$RT2, mu = 0.6)
#t = -20.839, p-value < 2.2e-16

t.test(data3$RT2, mu = 0.45)
#t = 8.3159, df = 5102, p-value < 2.2e-16
```
...
WRS2::yuen(RT2 ~ Gender, data = data3) # THe package WRS2 does not seem to be in my library even though I just installed it??
```
...
```{r}
round(pastecs::stat.desc(data3$RT2, basic = FALSE, norm = TRUE), digits = 2)
```
